...
def getDescription():
    ...
    with open('Medical_dataset/symptom_Description1.csv') as csv_file:
...
def getprecautionDict():
    ...
    with open('Medical_dataset/symptom_precaution1.csv') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        line_count = 0
        for row in csv_reader:
...
def get_bot_response():
    ...
    if session['step'] == "Description":
        ...
        if session["disease"] in description_list.keys():
            return description_list[session["disease"]] + " \n <br>  Precautions And Doctor And Hospital Recommendations"
        else:
